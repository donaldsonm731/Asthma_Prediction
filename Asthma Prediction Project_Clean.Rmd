...
The list above shows the MSE, R^2 and MAE for lm and glmnet. From this we feel that
the lm model is a better fit because it has a higher R^2 for just a little more 
MSE and MAE.

## Predict on Test Set
```{r}
###############################################################################
#WindSpeed + Humidity + Gender + OutdoorJob + Pressure + UVIndex data.frame(test[,c(4,5,8,9,11,12)])
library(caret)

model.lm = lm(ACTScore ~ WindSpeed + Humidity + Gender + Pressure + OutdoorJob+ UVIndex, data = train)
