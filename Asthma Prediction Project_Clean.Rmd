...
backmodel1 <- lmer(ACT ~ WindSpeed + Age + SmokingHabit + Humidity + Gender + OutdoorJob + OutdoorActivities + Pressure + UVIndex + (1|UserNo.) + (1|Location) + (0 + Gender|UserNo.) + (0 + Gender|Location) + (0 + Age|UserNo.) + (0 + Age|Location), data = train)
...
model_l = glmmLasso(ACTScore~ WindSpeed + Humidity + Temperature + as.factor(Gender) + as.factor(OutdoorJob) + as.factor(Pressure) + as.factor(Age) + as.factor(UVIndex),rnd = list(UserNo.=~1,Location=~ 1), lambda = 0.4132323, data = train,method="REML",switch.NR = TRUE,final.re = TRUE)
...
model_sb = lm(ACT ~ WindSpeed + Humidity + Gender + OutdoorJob + OutdoorActivities + Pressure + UVIndex, data =train)
#summary(model_sb)

#summary(model_l)

model3 = glmm(ACT~ WindSpeed + Humidity + Gender + OutdoorJob + OutdoorActivities + Pressure + UVIndex + (1|UserNo.), data = train)
...
#pred_3 =  predict(model3, validation, interval='predict')
#error_3 =  round(pred_3[,1]) - validation$ACTScore
...
#data.frame(R2 = R2(pred_3, validation $ ACTScore), 
#           RMSE = RMSE(pred_3, validation $ ACTScore), 
#           MAE = MAE(pred_3, validation $ ACTScore))
