...
Attempting to expand on the lasso part by running it with the mixed effects, not just the fixed effects to compare feature selection.
...
glm1 =  try(glmmLasso(ACTScore~ WindSpeed + Temperature + as.factor(Gender) + as.factor(Pressure) + as.factor(UVIndex) + as.factor(Age) + as.factor(SmokingHabit) + as.factor(OutdoorJob) +  Humidity,rnd = list(UserNo.=~1,Location=~ 1), lambda=0.001, data = train,switch.NR = TRUE,
 final.re = TRUE))
...
model = glmmLasso(ACTScore~ WindSpeed + Temperature + as.factor(Gender) + as.factor(Pressure) + as.factor(UVIndex) + as.factor(Age) + as.factor(SmokingHabit) + as.factor(OutdoorJob) +  Humidity,rnd = list(UserNo.=~1,Location=~ 1), lambda = 0.001, data = train,switch.NR = TRUE,
 final.re = TRUE)
