...
hum = scale(validation$Humidity, scale = TRUE)
temp = scale(validation$Temperature, scale = TRUE)
wind = scale(validation$WindSpeed, scale = TRUE)
...
```{r}
backmodel1 <- lmer(ACT ~ train$WindSpeed + hum + gender + ODJ + ODA + pressure + uv + (1|subject) + (1|loc) + (0 + gender|subject) + (0 + gender|loc) + (0 + age|subject) + (0 + age|loc), data = train)
