...
model_l = glmmLasso(ACTScore~ WindSpeed + Humidity + Temperature + as.factor(Gender) + as.factor(OutdoorJob) + as.factor(Pressure) + as.factor(Age) + as.factor(UVIndex),rnd = list(UserNo.=~1,Location=~ 1), lambda = 0.4132323, data = train,switch.NR = TRUE,final.re = TRUE)
