...
#asthmaData = predict.preProcess(asthmaData,method = c("center", "scale"))
```
...
## USE below if we think we need to keep some subjects out
#folds <- groupKFold(asthmaData$UserNo., k = 6)
#folds
```
...
The first model uses as many inputs as it can without getting a singular error and a random effect from the subjects. Model 2, same as 1 but now random effect is location. Model 3 also R.E. is location but with less varaibles.
...
model2 = lmer(ACT ~ gender + age + ODJ + ODA + smoking +  pressure  + temp + hum +  (1|loc), data = train )
...
# Covar matrix of all the fixed effects
#vcov(fm1)
