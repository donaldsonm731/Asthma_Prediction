...
- Look at all the different combinations of the categorical data to see if it
  gives a better understanding of the outliers and which combinations don't have 
  a lot of data. 
- look into mixture model


Questions to answer: 

- What are our outliers? -> we look at the categories againest the cont. data
                            and boxplot to find outliers outside of 1.5IQR
...
Sources:
 - http://www.unige.ch/ses/sococ/cl/r/bapr.e.html
...
names = names(asthmaData[,c(-6,-8,-9,-11)])
...
```{r}
summary(lm(ACT ~ temp + hum + wind + gender + age + ODJ + ODA + smoking + pressure + uv))
```
...
```{r}
summary(lm(ACT ~ hum + wind + gender + age + ODJ + ODA + smoking  + uv))
...
# no parameter tuning
set.seed(5)
fit = lm(ACTScore ~ ., data = asthmaData)
...
```{r}
bestmodel <- lm(ACT ~ hum + wind + age + ODA + pressure + uv)
plot(bestmodel)
```
