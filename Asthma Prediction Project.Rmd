...
Questions to answer: 
- If using PCA, can we then use univariate outlier detection methods?
- What are our outliers?
- Can we find the minimum in the bias variance trade off?
- Other prediction methods to explore: Poly Regression, Random Forest or regular decision trees, SVM... 
        

## Practicum of Data Analytics Project
################################################################################
First part is just importing and data visualization
```{r}
...
#End of visualization of the data


#################################################################################
## Finding outliers in the categorical/numeric data
To do this we need to go through all the possibilities from the inputs with categorical data and do a box a whisker with the continuous inputs.

first plotting 1 cat at a time with temp humidity
```{r}
...
```{r}
boxplot(asthmaData$Temperature~asthmaData$Age,data=asthmaData)
boxplot(asthmaData$Temperature~asthmaData$Gender,data=asthmaData)
boxplot(asthmaData$Temperature~asthmaData$SmokingHabit,data=asthmaData)
boxplot(asthmaData$Temperature~asthmaData$OutdoorJob,data=asthmaData)
boxplot(asthmaData$Temperature~asthmaData$OutdoorActivities,data=asthmaData)
boxplot(asthmaData$Temperature~asthmaData$Pressure,data=asthmaData)
boxplot(asthmaData$Temperature~asthmaData$UVIndex,data=asthmaData)

```

lastly, plotting 1 cat at a time with windspeed
```{r}
boxplot(asthmaData$WindSpeed~asthmaData$Age,data=asthmaData)
boxplot(asthmaData$WindSpeed~asthmaData$Gender,data=asthmaData)
boxplot(asthmaData$WindSpeed~asthmaData$SmokingHabit,data=asthmaData)
boxplot(asthmaData$WindSpeed~asthmaData$OutdoorJob,data=asthmaData)
boxplot(asthmaData$WindSpeed~asthmaData$OutdoorActivities,data=asthmaData)
boxplot(asthmaData$WindSpeed~asthmaData$Pressure,data=asthmaData)
boxplot(asthmaData$WindSpeed~asthmaData$UVIndex,data=asthmaData)

```
```{r}
index_smoking = which(asthmaData$SmokingHabit == 0)
index_gender = which(asthmaData$Gender == 0)
index_age = which(asthmaData$Age == 1)
index_outDoorAct = which(asthmaData$OutdoorActivities == 1)
index_outDoorJob = which(asthmaData$OutdoorJob == 1)
for (j in min(asthmaData$Pressure):max(asthmaData$Pressure)){ 
for (i in 0:1){ 
index_UV = which(asthmaData$UVIndex == i)
index_pressure = which(asthmaData$Pressure == j)

index = intersect(index_smoking,intersect(index_gender,intersect(index_age, x =   intersect(index_outDoorAct,intersect(index_outDoorJob,intersect(index_UV,index_pressure))))))
print(index)
