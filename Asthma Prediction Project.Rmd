...
train = asthmaData.pca[dt,]
test = asthmaData.pca[dt,]
...
train_fit = lm(y ~ v1 + v2 + v3 + v4 + v5 + v6 + v7)

summary(train_fit)
```
```{r}
#Residuals
resid = train_fit$residuals
#R^2 value
r_sq = summary(train_fit)$r.squared
# F-Test Statistic
F_stat = summary(train_fit)$fstat
# C.I. for regression CO
coef_CI = confint(train_fit)
...
vcov(train_fit) # Last one is interesting to see, bc using PCA they should be around 0
```

## Predicting Values
```{r}
b0 = train_fit$coefficients[1]
b = train_fit$coefficients[2:8]
x = test[1,1:7]
y_pred = b0 + b %*% x
```
...
newdata = data.frame(test[,1:7])
predictions = predict(train_fit,newdata,interval = "prediction")
```
