...
before finding the outliers using boxplot (finds points outside 1.5*IQR), we will
first get a sense of how big each category is, so incase a category is to small,
we will keep that instance in.
...
# of them out
totOutlier_H = unique(c(outlier_ageH, outlier_genderH, outlier_smH, outlier_ODJH, outlier_ODAH,
                 outlier_pH, outlier_UVH))
...
# number once, so say 114 is stored twice then the unique() will just take one 
# of them out
totOutlier_T= unique(c(outlier_ageT, outlier_genderT, outlier_smT, outlier_ODJT, outlier_ODAT,
                 outlier_pT, outlier_UVT))
...
# number once, so say 114 is stored twice then the unique() will just take one 
# of them out
totOutlier_W = unique(c(outlier_ageW, outlier_genderW, outlier_smW, outlier_ODJW, outlier_ODAW,
                 outlier_pW, outlier_UVW))
...
```{r}
totOutliers = unique(c(totOutlier_H, totOutlier_T, totOutlier_W))
totOutliers
...
if(dim(asthmaData)[1] == dim(asthmaDataOriginal)[1]){
  asthmaData_reduced = asthmaData[-c(totOutliers),]
}
