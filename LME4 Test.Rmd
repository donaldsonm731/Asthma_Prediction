...
modelCombs = combine_predictors(
  dependent = "score",
  fixed_effects = c("temp","hum","gender","pres","uv"),
...
#for (i in 1:length(modelCombs)){
#  if (grepl("*", modelCombs[i])){
...
asthmaData$age = factor(asthmaData$age)
...
mixed_model_formulas <- c("score ~ 1",
                          ...
                          "score ~ temp + hum  + uv + ws + gender + odj + (1|subj) + (1|loc)")
...
mixed_model_formulas <- c("score ~ age ",
                          "score ~ gender + (1|subj) + (1|loc)")
